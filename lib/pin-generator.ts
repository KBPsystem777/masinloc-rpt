...
export interface Property {
  ...
  area: number
  owner: string
  status: "Active" | "Subdivided" | "Consolidated"
  parentPin?: string
  createdAt: Date
}
...
export function generatePIN(provinceCode: string, townCode: string, blockNumber: number, lotNumber: number): string {
...
export function getNextLotNumber(properties: Property[], blockNumber: number): number {
  const lotsInBlock = properties.filter((p) => p.blockNumber === blockNumber && p.status === "Active")
  if (lotsInBlock.length === 0) return 1
  const maxLot = Math.max(...lotsInBlock.map((p) => p.lotNumber))
  return maxLot + 1
}

export function canAddLotToBlock(properties: Property[], blockNumber: number): boolean {
  const lotsInBlock = properties.filter((p) => p.blockNumber === blockNumber && p.status === "Active")
